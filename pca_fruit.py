...
def getYourFruits(fruits, data_type, print_n=False, k_fold=False):
    ...
    val = ['Training', 'Test']
    if not k_fold:
        path = "../fruit_all/*/fruits-360/" + data_type + "/"
...
def getAllFruits():
    fruits = []
    for fruit_path in glob.glob("../fruit_all/*/fruits-360/Training/*"):
        fruit = fruit_path.split("/")[-1]
        fruits.append(fruit)
    return fruits

#Choose your Fruits
fruits = ['Apple Golden 1' , 'Pear'] #Binary classification
    ...
    if (dim == 2):
        for i, index in enumerate(v):
            ...
            plt.scatter(X[start:end, 0], X[start:end, 1], color=colors[i % len(colors)],
                        marker=markers[i % len(markers)], label=fruits[i])
            start = end
        plt.xlabel('PC1')
        plt.ylabel('PC2')
